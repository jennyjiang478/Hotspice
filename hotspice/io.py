...
# Redefine the FieldInputter class with the modifications
class FieldInputter(Inputter):
    def __init__(self, datastream: Datastream, magnitude=1, angle=0, n=2, sine=False, frequency=1, half_period=False):
    ...
    def input_single(self, mm: Magnets, value: float|int, sub_steps=10):
        ...
        if self.sine: # Change magnitude sinusoidally
            while lower_than(progress := max((mm.MCsteps - MCsteps0)/self.n, (mm.t - t0)*self.frequency), 1):
                ...
                for _ in range(sub_steps):  # More frequent updates
                    mm.update(t_max=min(1-progress, 0.05)/self.frequency/sub_steps) # Split update into sub-steps
        else: # Use constant magnitude
            Zeeman.set_field(magnitude=self.magnitude*value)
            mm.progress(t_max=1/self.frequency/(2*period)/sub_steps, MCsteps_max=self.n/(2*period)/sub_steps)
            if self.full_period:
                Zeeman.set_field(magnitude=-self.magnitude*value)
                mm.progress(t_max=.5/self.frequency/sub_steps, MCsteps_max=self.n/2/sub_steps)
...
class FieldInputterBinary(FieldInputter):
    ...
    def input_sine_data(self, mm: Magnets, value: bool|int, data=None):
        data = super().input_sine_data(mm, 1 if value else self.magnitude_ratio, data=data)
        return data
